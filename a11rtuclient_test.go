...
func TestA11RTUClientProvider_encodeRTUFrame(t *testing.T) {
	type args struct {
		slaveID byte
...
func TestA11RTUClientProvider_decodeRTUFrame(t *testing.T) {
	type args struct {
		adu []byte
	}
	tests := []struct {
...
func Test_a11verify(t *testing.T) {
	type args struct {
		reqSlaveID uint8
		rspSlaveID uint8
	...
	tests := []struct {
		...
		{
			"serial a11verfy slaveID different",
			args{
				4,
				5,
				ProtocolDataUnit{10, []byte{1, 2, 3, 4}},
		...
		{
			"serial a11verfy functionCode different",
			args{
				5,
				5,
		...
		{
			"serial a11verfy pdu data zero length",
			args{
				5,
				5,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("a11verfy() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func Test_a11calculateResponseLength(t *testing.T) {
	type args struct {
		adu []byte
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := a11calculateResponseLength(tt.args.adu); got != tt.want {
...
func A11BenchmarkRTUClientProvider_encodeRTUFrame(b *testing.B) {
...
func A11BenchmarkRTUClientProvider_decodeRTUFrame(b *testing.B) {
