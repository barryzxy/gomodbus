...
const (
	a11rtuExceptionSize = 5
)

// RTUClientProvider implements ClientProvider interface.
type A11RTUClientProvider struct {
	serialPort
	logger
	*pool
}
...
var _ ClientProvider = (*A11RTUClientProvider)(nil)

// request pool, all RTU client use this pool
var a11rtuPool = newPool(rtuAduMaxSize)
...
func NewA11RTUClientProvider(opts ...ClientProviderOption) *A11RTUClientProvider {
	p := &A11RTUClientProvider{
...
func (sf *protocolFrame) a11encodeRTUFrame(slaveID byte, pdu ProtocolDataUnit) ([]byte, error) {
...
// decode extracts slaveID and PDU from RTU frame and a11verfy CRC.
func a11decodeRTUFrame(adu []byte) (uint8, []byte, error) {
...
func (sf *A11RTUClientProvider) Send(slaveID byte, request ProtocolDataUnit) (ProtocolDataUnit, error) {
	...
	err = a11verfy(slaveID, rspSlaveID, request, response)
	return response, err
}

// SendPdu send pdu request to the remote server
func (sf *A11RTUClientProvider) SendPdu(slaveID byte, pduRequest []byte) ([]byte, error) {
	...
	response := ProtocolDataUnit{pdu[0], pdu[1:]}
	if err = a11verfy(slaveID, rspSlaveID, request, response); err != nil {
		return nil, err
	}
...
func (sf *A11RTUClientProvider) SendRawFrame(aduRequest []byte) (aduResponse []byte, err error) {
	sf.mu.Lock()
	defer sf.mu.Unlock()
	...
	bytesToRead := a11calculateResponseLength(aduRequest)
	time.Sleep(sf.a11calculateDelay(len(aduRequest) + bytesToRead))
	...
	case data[1] == functionFail:
		...
		if n < a11rtuExceptionSize {
			n1, err = io.ReadFull(sf.port, data[n:a11rtuExceptionSize])
		}
		n += n1
	default:
...
func (sf *A11RTUClientProvider) a11calculateDelay(chars int) time.Duration {
...
func a11calculateResponseLength(adu []byte) int {
...
// a11verfy confirms valid data(including slaveID,funcCode,response data)
func a11verfy(reqSlaveID, rspSlaveID uint8, reqPDU, rspPDU ProtocolDataUnit) error {
	switch {
